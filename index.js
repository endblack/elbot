...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\ seja bem vindo/a ao grupo *${mdata.subject}* comportasse para não ganha ban 😳`
				...
				teks = `Adeus @${num.split('@')[0]} vai com o diabo e um beijinho no cu`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
case 'lofi':
			...
			case 'mapa': 
				...
				case 'tts':
				case 'audio':
					if (args.length < 1) return client.sendMessage(from, 'qual o código do idioma tio??', text, {quoted: mek})
				...
				case 'adicionar':
					...
					if (args[0] === 'abrir') {
						...
						reply(`grupo fechado SOMENTE ADM'S MANDA MSGS NESSA PORRA`)
            ...
            case 'criador':
            case 'keke':
                  client.sendMessage(from, {displayname: "Max admin", vcard: vcard}, MessageType.contact, { quoted: mek})
